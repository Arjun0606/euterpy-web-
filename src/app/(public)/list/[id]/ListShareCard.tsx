...
export default function ListShareCard({ listId, title, username }: Props) {
  const [downloading, setDownloading] = useState(false);
  const cardUrl = `/api/og/list/${listId}?ts=${Date.now()}`;
  ...
  async function handleDownload() {
    ...
    try {
      ...
      link.href = url;
      link.download = `${username}-${title.replace(/[^a-z0-9]/gi, "-").toLowerCase()}.png`;
  ...
  return (
    <section>
      <p className="text-[11px] uppercase tracking-[0.18em] text-zinc-500 mb-5">— Share this list</p>
      <div className="bg-card border border-border rounded-2xl p-5">
        <div className="rounded-xl overflow-hidden mb-5 border border-border bg-black aspect-[1080/1350]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={cardUrl} alt="List card" className="w-full h-full object-contain" />
        </div>
