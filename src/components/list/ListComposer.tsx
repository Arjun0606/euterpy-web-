...
export default function ListComposer({ existing, onClose }: Props) {
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      ...
      const albums: SearchResult[] = (data.albums || []).slice(0, 5).map((a: any) => ({ ...a, kind: "album" }));
      const songs: SearchResult[] = (data.songs || []).slice(0, 5).map((s: any) => ({ ...s, kind: "song" }));
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isEdit) {
        ...
        // Wipe and recreate items (simplest correct path)
      ...
      toast(isEdit ? "List saved" : "List published");
      onClose();
  ...
  async function handleDelete() {
    ...
    try {
      const supabase = createClient();
      const { error } = await supabase.from("lists").delete().eq("id", existing!.id);
      if (error) throw error;
      toast("List deleted");
      onClose();
      router.refresh();
    ...
    <div className="fixed inset-0 z-50 flex items-stretch sm:items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          ...
          <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-600">{isEdit ? "Edit list" : "New list"}</p>
          <button
            onClick={handleSave}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 sm:px-10 sm:py-10">
          ...
          <input
            ...
            maxLength={200}
            placeholder="List title — be evocative"
            className="w-full bg-transparent border-none text-3xl sm:text-5xl font-display tracking-tighter text-white placeholder:text-zinc-700 focus:outline-none mb-3"
            autoFocus={!isEdit}
          />
          <input
            type="text"
            ...
            className="editorial w-full bg-transparent border-none text-base text-zinc-400 placeholder:text-zinc-700 focus:outline-none mb-8 italic"
          />

          {/* Items */}
          {items.length > 0 && (
            <div className="mb-8 space-y-2">
              <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600 mb-3">— {items.length} {items.length === 1 ? "item" : "items"}</p>
              {items.map((item, idx) => (
                <div
          ...
          {/* Search */}
          <div>
            <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600 mb-3">— Add to list</p>
            <input
              type="text"
              value={query}
        ...
        {!isEdit && (
          <div className="px-6 py-4 border-t border-border text-center">
            <p className="text-[10px] text-zinc-700">Drag items to reorder</p>
          </div>
        )}
      </div>
    </div>
